...
const Performance = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post("http://127.0.0.1:5000/predict1", {
        features: [
          Number(formData['Attendance (%)']),
          Number(formData['Internal Test 1 (out of 40)']),
          Number(formData['Internal Test 2 (out of 40)']),
          Number(formData['Assignment Score (out of 10)']),
          Number(formData['Daily Study Hours'])
        ]
      });
  ...
  const inputs = [
    { label: "Attendance (%)", name: "Attendance (%)", icon: CalendarCheck, type: "number", min: 0, max: 100, placeholder: "52 - 100" },
    { label: "Internal Test 1", name: "Internal Test 1 (out of 40)", icon: Book, type: "number", min: 0, max: 40, placeholder: "0 - 40" },
    { label: "Internal Test 2", name: "Internal Test 2 (out of 40)", icon: GraduationCap, type: "number", min: 0, max: 40, placeholder: "0 - 40" },
    { label: "Assignment Score", name: "Assignment Score (out of 10)", icon: Target, type: "number", min: 0, max: 10, placeholder: "0 - 10" },
    { label: "Daily Study Hours", name: "Daily Study Hours", icon: Clock, type: "number", min: 1, max: 12, placeholder: "1 - 5" },
  ];
    ...
    <div className="min-h-full py-8 px-4 sm:px-6 lg:px-8 bg-slate-50/50">
      ...
      >
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          {report && (
            <motion.div variants={itemVariants} className={`hidden lg:flex items-center space-x-2 px-4 py-2 rounded-lg font-medium border ${report.category === 'High' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
            ...
            <div className="lg:col-span-2 bg-slate-50 p-6 sm:p-8 flex flex-col justify-center min-h-[350px]">
              ...
              <div className="h-full flex flex-col justify-center">
                {report ? (
                  ...
                  >
                    <div className={`mx-auto w-20 h-20 rounded-full flex items-center justify-center mb-5 border-[6px] border-white shadow-sm ${report.category === 'High' ? 'bg-emerald-100' :
                        report.category === 'Medium' ? 'bg-amber-100' : 'bg-red-100'
                      }`}>
                      <span className={`text-lg font-bold ${report.category === 'High' ? 'text-emerald-700' :
